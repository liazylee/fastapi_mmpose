...
class FrameCollector:
    ...
    def add_frame(self, frame: np.ndarray) -> FrameData:
        ...
        logger.debug(f"Frame {frame_data.frame_id} (seq:{self.sequence_counter}) added to buffer, size: {len(self.frame_buffer)}")
        return frame_data
...
class BatchProcessor:
    ...
    async def _process_batch(self, batch_frames: List[FrameData]) -> List[ProcessedResult]:
        ...
        # Process batch
        start_time = time.time()
        results = await self.processor.process_batch(batch_items)
        processing_time = time.time() - start_time
...
class ResultManager:
    ...
    def __init__(self, max_results: int = 20):
        ...
        self.max_sequence_gap = 10  # Maximum gap to wait for missing frames
    ...
    def get_next_sequential_frame(self) -> Tuple[Optional[np.ndarray], Optional[Any], bool]:
        ...
        if next_seq in self.ordered_results:
            # Found the next frame in sequence
        ...
        # Check if we should skip ahead (frame might be lost)
        ...
        if available_sequences:
            ...
            if gap <= self.max_sequence_gap:
                # Gap is small, wait for missing frames
                logger.debug(f"Waiting for sequence {next_seq}, next available: {min_available}")
                return (None, None, False)
            else:
                # Gap is too large, skip ahead to avoid accumulating delay
                logger.warning(f"Skipping frames {next_seq} to {min_available-1} due to large gap")
                self.output_sequence_number = min_available - 1
                return self.get_next_sequential_frame()  # Recursive call to get the frame
        
        # No frames available in sequence
        return (None, None, False)
...
class StreamOutputController:
    ...
    def should_output_frame(self) -> bool:
        """Check if it's time to output a new frame"""
...
class StreamingPoseProcessor:
    ...
    def __init__(self, config: BatchProcessingConfig = None, target_fps: int = 30):
        ...
        self.frame_collector = FrameCollector(max_buffer_size=15)  # Reduced from 30
        self.batch_processor = BatchProcessor(self.config)
        self.result_manager = ResultManager(max_results=10)  # Reduced from 20
